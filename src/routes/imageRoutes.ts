    ...
    if(queries.image){
      if(fs.existsSync( path.join( 'image', queries?.image as string ) )){
        if(!queries.width && !queries.height){
          ...
          if(fs.existsSync( path.join( 'output', `${fileName[0]}-w${queries.width}-h${queries.height}.${fileName[1]}` ) )){
            res.sendFile(path.resolve( 'output', `${fileName[0]}-w${queries.width}-h${queries.height}.${fileName[1]}` ))
          }else{
            if(isNaN((queries.width as unknown) as number) && queries.width){
              res.send('width should be Number Only!')
            }else
            if(isNaN((queries.height as unknown) as number) && queries.height){
